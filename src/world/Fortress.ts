...
export class Fortress {
  readonly name = 'Fortress';
  readonly title = 'The Fortress';
  ...
  constructor(
    ...
    for (const s of [-1, 1]) {
      ...
      const bp = at(s * 26, 6);
      const bunker = new Bunker(world, scene, hitRegistry, bp.x, bp.z, yaw + Math.PI, 'enemy', color);
      ...
      for (const lz of [-80, 80]) {
        const gb = at(s * 26, lz);
        const guard = new Bunker(world, scene, hitRegistry, gb.x, gb.z, yaw + (lz > 0 ? Math.PI : 0), 'enemy', color);
  ...
  private buildGround(ground: number, yaw: number): void {
    ...
    flat(16, WALL * 2 + 40, 0x45484d, 0, 0, -3);
    for (let z = -WALL; z < WALL; z += 10) flat(0.4, 5, 0xe8d36a, 0, z + 2.5, -4);
  }
  ...
  private buildWalls(ground: number, yaw: number): void {
    ...
    for (const x of [-WALL, WALL]) {
      for (const z of [-WALL, WALL]) {
        ...
        p.add(new THREE.CylinderGeometry(0.12, 0.15, 8, 6), dark, x, 17.6, z);
    ...
    const signMat = new THREE.MeshStandardMaterial({ map: signTexture('THE FORTRESS') });
    for (const z of [-WALL, WALL]) {
      ...
      for (const s of [-1, 1]) {
        ...
        leaf.add(new THREE.BoxGeometry(GATE_HALF, 8.5, 1), plastic(0x5b5f58), 0, 4.25, 0);
